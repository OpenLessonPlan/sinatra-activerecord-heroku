...
foreach ($dirs as $dir) {
    if (!is_writable($dir)) {
        if (!chmod($dir, 0755)) {
            $allErrors[] = $dir . " is not writeable. This is needed for the install to work.";
        }
    }
}
...
if (isset($_POST) && !empty($_POST)) {
    ...
    if ((!empty($_POST["host"]) && !empty($_POST["database"]) && !empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["tablePrefix"]) && !empty($_POST["admin_username"]) && !empty($_POST["admin_password"]) && !empty($_POST["admin_password_again"]) && !empty($_POST["fileSize"]) && !empty($_POST["filetype"])) && ($_POST["admin_password"] == $_POST["admin_password_again"])) {
...
<html>
    ...
    <body>
        ...
        <?php if (!file_exists("config/config.php")) { ?>
            <!-- SETUP -->
            <form class="form-horizontal" method="post" action="<?php echo htmlentities($_SERVER["PHP_SELF"], ENT_QUOTES); ?>" id="mainForm">
                <div class="container">
                    <div class="row">
                        ...
                        <div class="container">
                            <div class="installHeader col-lg-12 text-center">
                                ...
                                <h3 class="text-left paddingLeft5"><small>Installation file has been deleted and you can now start uploading your images. <a href="start.php" class="alert-link">CleanUploader</a></small></h3>
                            </div>
                        </div>
